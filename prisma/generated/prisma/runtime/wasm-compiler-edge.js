      ...
      s = (a) => {
         a.forEach((f) => {
            f.includes('Uint') && (e[f.replace('Uint', 'UInt')] = e[f]),
               f.includes('Float64') &&
                  (e[f.replace('Float64', 'Double')] = e[f]),
               f.includes('Float32') &&
                  (e[f.replace('Float32', 'Float')] = e[f]);
         });
      };
   n.forEach((a, f) => {
      a.startsWith('read') &&
      ...
      h = class e extends Uint8Array {
         ...
         write(t, r, n, i = 'utf8') {
            let o = typeof r == 'string' ? 0 : r ?? 0,
               s =
                  typeof n == 'string' ? this.length - o : n ?? this.length - o;
            return (
         ...
         toString(t = 'utf8', r = 0, n = this.length) {
            if (((r = r < 0 ? 0 : r), (t = t.toString().toLowerCase()), n <= 0))
               return '';
            if (t === 'utf8' || t === 'utf-8')
               return du.decode(this.slice(r, n));
            if (t === 'base64' || t === 'base64url') {
...
function Ko(e, t) {
   ...
   if (
      ...
      for (
         i = f.length, a = R.length, A = i < a, A && (a = i), i = 0;
         i < a;
         i++
      )
         if (f[i] != R[i]) {
      ...
      if (e.dependentArgumentPath) {
         ...
         t.addErrorMessage(
            (E) =>
               `Argument \`${E.green(
                  o
               )}\` is required because argument \`${E.green(
                  f
               )}\` was provided.`
         );
      }
...
function Gi(e) {
   throw nn(e)
      ? new ce(
           ...
           }\`. Message: \`${e.cause.originalMessage ?? Ya(e)}\``,
           'P2010',
...
function Ya(e) {
   switch (e.cause.kind) {
      ...
      case 'ForeignKeyConstraintViolation':
         return `Foreign key constraint violated on the ${Hi(
            e.cause.constraint
         )}`;
      case 'UnsupportedNativeDataType':
...
function Xa(e, t, r) {
   ...
   for (let [i, o] of Object.entries(t))
      switch (o.type) {
         ...
         case 'object': {
            if (
               o.serializedName !== null &&
               !Object.hasOwn(e, o.serializedName)
            )
               throw new G(
...
function Ad(e, t = Zl) {
   ...
   if (e < 0)
      throw new ot(it.EncodeTimeNegative, `Time must be positive: ${e}`);
   if (Number.isInteger(e) === !1)
      throw new ot(
         it.EncodeTimeValueMalformed,
   ...
   switch (n) {
      ...
      case 'parameterTuple':
         return `(${
            e.value.length == 0
               ? 'NULL'
               : e.value.map(() => bo(t, r.placeholderNumber++)).join(',')
         })`;
   ...
   async interpretNode(t, r, n, i) {
      switch (t.type) {
         ...
         case 'concat': {
            ...
            return {
               value:
                  o.length > 0 ? o.reduce((s, a) => s.concat(To(a)), []) : [],
            };
         }
         case 'sum': {
            let o = await Promise.all(
            ...
            return {
               value: o.length > 0 ? o.reduce((s, a) => Oe(s) + Oe(a)) : 0,
            };
         }
         case 'execute': {
         ...
         case 'join': {
            ...
            let a = await Promise.all(
               t.args.children.map(async (f) => ({
                  joinExpr: f,
                  childRecords: (
                     await this.interpretNode(f.child, r, n, i)
                  ).value,
               }))
            );
...
${n.backtrace}`,
   ...
   async request(t, { interactiveTransaction: r, customDataProxyFetch: n }) {
      ...
      try {
         a = this.#u(() =>
            this.#d({ queries: [t], execute: () => s.compile(i) })
         );
      } catch (f) {
         throw this.#p(f);
      }
         ...
         switch (f.type) {
            ...
            case 'compacted': {
               if (!t.every((C) => C.action === i))
                  throw new Error(
                     'All queries in a batch must have the same action'
                  );
      ...
      if (!R.ok)
         throw new Error(
            `Failed to fetch stable Prisma version, unpkg.com status ${
               R.status
            } ${R.statusText}, response body: ${
               (await R.text()) || '<empty body>'
            }`
         );
      let S = await R.text();
   ...
   Gr = class {
      ...
      propagateResponseExtensions(t) {
         t?.logs?.length &&
            ...
            t?.traces?.length &&
               this.tracingHelper.dispatchEngineSpans(t.traces);
      }
      onBeforeExit() {
         throw new Error(
      ...
      async requestBatch(
         ...
         return (
            ...
            (f) => (
               ...
               'errors' in f
                  ? this.convertProtocolErrorsToClientError(f.errors)
                  : f
            )
         );
      }
      ...
      async withRetry(t) {
         for (let r = 0; ; r++) {
            ...
            try {
               ...
               this.logEmitter.emit('warn', {
                  message: `Attempt ${r + 1}/${vc} failed for ${
                     t.actionGerund
                  }: ${i.message ?? '(unknown)'}`,
                  timestamp: new Date(),
                  target: '',
         ...
         if (e) {
            ...
            for (let [r, n] of Object.entries(e)) {
               ...
               if (typeof n != 'object' || Array.isArray(n))
                  throw new V(`Invalid value ${JSON.stringify(
                     e
